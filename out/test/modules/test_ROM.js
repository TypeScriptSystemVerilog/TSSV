...
console.log('test 1');
mkdirSync('sv-examples/ROM/ROM_32_big_interal', { recursive: true });
const testMem1 = new ROM({ dataWidth: 32 }, 'ts/test/test_rom_init_file.txt', 'sv-examples/ROM/ROM_32_big_interal');
try {
    writeFileSync(`sv-examples/ROM/${testMem1.name}/${testMem1.name}.sv`, testMem1.writeSystemVerilog());
}
catch (err) {
    console.error(err);
}
...
try {
    mkdirSync(`sv-examples/ROM/${testMem2.name}`, { recursive: true });
    writeFileSync(`sv-examples/ROM/${testMem2.name}/${testMem2.name}.sv`, testMem2.writeSystemVerilog());
}
catch (err) {
    console.error(err);
}
...
try {
    mkdirSync(`sv-examples/ROM/${testMem3.name}`, { recursive: true });
    writeFileSync(`sv-examples/ROM/${testMem3.name}/${testMem3.name}.sv`, testMem3.writeSystemVerilog());
}
catch (err) {
    console.error(err);
}
...
console.log('test 4');
const test_array_dec = [-5, -255, 0, 255, 32767, -32767];
const testMem4 = new ROM({ name: 'ROM_16_6_normal_array_test', dataWidth: 16 }, test_array_dec);
try {
    mkdirSync(`sv-examples/ROM/${testMem4.name}`, { recursive: true });
    writeFileSync(`sv-examples/ROM/${testMem4.name}/${testMem4.name}.sv`, testMem4.writeSystemVerilog());
}
catch (err) {
    console.error(err);
}
...
try {
    mkdirSync(`sv-examples/ROM/${testMem5.name}`, { recursive: true });
    writeFileSync(`sv-examples/ROM/${testMem5.name}/${testMem5.name}.sv`, testMem5.writeSystemVerilog());
}
catch (err) {
    console.error(err);
}
...
try {
    mkdirSync(`sv-examples/ROM/${testMem6.name}`, { recursive: true });
    writeFileSync(`sv-examples/ROM/${testMem6.name}/${testMem6.name}.sv`, testMem6.writeSystemVerilog());
}
catch (err) {
    console.error(err);
}
...
try {
    mkdirSync(`sv-examples/ROM/${testMem7.name}`, { recursive: true });
    writeFileSync(`sv-examples/ROM/${testMem7.name}/${testMem7.name}.sv`, testMem7.writeSystemVerilog());
}
catch (err) {
    console.error(err);
}
...
console.log('test 8');
const array_item_ExceedRomWidth = [-5, -255, 0, 255, 511];
const testMem8 = new ROM({ name: 'ROM_9_5_normal_array_test', dataWidth: 9 }, array_item_ExceedRomWidth);
try {
    mkdirSync(`sv-examples/ROM/${testMem8.name}`, { recursive: true });
    writeFileSync(`sv-examples/ROM/${testMem8.name}/${testMem8.name}.sv`, testMem8.writeSystemVerilog());
}
catch (err) {
    console.error(err);
}
...
try {
    mkdirSync(`sv-examples/ROM/${testMem9.name}`, { recursive: true });
    writeFileSync(`sv-examples/ROM/${testMem9.name}/${testMem9.name}.sv`, testMem9.writeSystemVerilog());
}
catch (err) {
    console.error(err);
}
