...
try {
    mkdirSync('sv-examples/Core/multiFileEmission', { recursive: true });
}
catch (e) { }
let passed = 0;
let failed = 0;
...
check('single-file emission defines the whole design', definitions(baseline).has('dut') && definitions(baseline).has('leaf') &&
    definitions(baseline).has('agent') && definitions(baseline).has('tb') &&
    definitions(baseline).has('memory_32_32'), `defined: ${sorted(definitions(baseline))}`);
...
check('together the files define everything the single file did', missing.length === 0, `missing: ${missing.join(', ')}`);
check('the testbench file still instantiates the excluded DUT', /^\s+dut\s+\S*\s*u_dut/m.test(tbSV));
// ---- Test 3: `defined` reports the emitted module itself ----------------------------------
check('`defined` includes the emitted module\'s own name', defined.has('dut'), `defined: ${sorted(defined)}`);
check('`defined` includes its submodules and interfaces', defined.has('leaf') && defined.has('memory_32_32'), `defined: ${sorted(defined)}`);
...
check('the shared interface is defined in the first file emitted', inDut.has('memory_32_32') && !inTb.has('memory_32_32'));
const tbFirstDefined = new Set();
const tbFirst = makeTb(new Dut());
tbFirst.writeSystemVerilog({ defined: tbFirstDefined });
check('emitting the testbench first would instead define it there', tbFirstDefined.has('memory_32_32'));
