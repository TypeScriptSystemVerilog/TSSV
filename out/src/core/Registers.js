import { Module, Expr, Sig } from 'tssv/lib/core/TSSV';
import { Memory } from 'tssv/lib/interfaces/Memory';
import { APB4 } from 'tssv/lib/interfaces/AMBA/AMBA4/APB4/r0p0_0/APB4';
import { Casex, generateAPBStr, calculateDecMask, replaceZerosWithX, AIGC2APB4, AIGCStr, padZeroes } from 'tssv/lib/tools/shared';
function isValidBusAddressWidth(x) {
...
export class RegisterBlock extends Module {
    ...
    setupSignalsExprs(regDefs, params, useComb = true, mappings = {}) {
        ...
        const signals = { reg_rd, reg_wr, reg_addr, reg_rdata, reg_wdata, next_rdata, inRange };
        const expressions = { next_rdataExpr, inRangeExpr };
        if (useComb) {
            this.addCombAlways({
    ...
    setupRegIO(regName, thisReg, pkSig, reg_wdata, WE_Sig) {
        ...
        if (!thisReg.useBuf) {
            ...
            const bufSig = this.addSignal(`buf_${regName}`, { width: thisReg.width || this.regDefs.wordSize, type: 'reg' });
    ...
    setWriteOneClearOrToggle(regName, pkSig, matchSig, thisReg, regDefs, type) {
    ...
    constructor(params, regDefs, busInterface) {
        ...
        if (busInterface instanceof Memory) {
            ...
            for (const reg in this.regDefs.addrMap) {
                ...
                const readSignal = { a: matchSig, b: thisReg.type === 'WO' ? '32\'h0' : ((thisReg.type === 'W1C' || thisReg.type === 'W1T' || thisReg.type === 'W0T' || thisReg.type === 'W0C') ? `${'reg_' + regName.toString()}` : pkSig) };
    ...
    genRegBlkMemory(regDefs, params) {
