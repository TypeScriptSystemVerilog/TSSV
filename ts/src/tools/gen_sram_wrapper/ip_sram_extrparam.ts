...
const logStream = fs.createWriteStream('ip_sram_convert.log', { flags: 'a' })
...
interface SramModule {
  ...
  instanceName: string
  frequency?: number
  port?: '1p10' | '1p11' | '2p11' | '2p12' | 'UNKNOWN'
  number?: number
  dataWidth: number
  addressWidth: number
  depth: number
...
function generatePredefinedConstants (verilogCode: string): void {
...
function extractSramModules (verilogCode: string): [string, SramModule[]] {
  ...
  if (rhcRegex.test(verilogCode)) {
    ...
    while ((instantiationMatch = instantiationRegex.exec(verilogCode)) !== null) {
      ...
      let port: '1p10' | '1p11' | '2p11' | '2p12' | 'UNKNOWN' = 'UNKNOWN'
    ...
    for (const match of matches) {
      ...
      let port: '1p10' | '1p11' | '2p11' | '2p12' | 'UNKNOWN' = '1p11'
...
function jsonToCsv (jsonData: SramModule[]): string {
  ...
  jsonData.forEach(sram => {
    const row = [
      ...
      sram.depth?.toString() || '',
      sram.total || '',
      sram.sw !== undefined && sram.sw !== 0 ? 1 : 0,
      sram.columnMux || '',
      sram.bank || '',
      sram.centerDecoder || '',
...
function main (): void {
  ...
  if (args.length !== 2) {
    console.error('Usage: node ip_sram_convert.js <path-to-verilog-file> <output-path>')
    process.exit(1)
  }
