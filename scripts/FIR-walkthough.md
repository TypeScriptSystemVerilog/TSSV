...
Creates a combinational multiplier. `a` is a `Sig` (the tap register), `b` is a `Sig` pointing to the coefficient register output. TSSV auto-computes the output bit width based on the operand widths and returns a `Sig` pointing to the product signal. Generates `assign prod_tapN_x_coeffN = tap_N * coeff_N;`. Because `b` is now a signal rather than a compile-time constant, the coefficients can be updated at runtime by writing to the register block (see Part 4).
...
## Part 4 — Generating a Register Block for Runtime-Configurable Coefficients

The FIR coefficients are hardcoded as `bigint[]` parameters at elaboration time, but TSSV also lets you back them with a `RegisterBlock` so a host processor can update them at runtime over a Memory bus. The `gen_regblock.sh` script turns a YAML description into the boilerplate TypeScript you need.

---

### Step 1: Write the YAML register map

Create a YAML file **in the same directory as the module** (convention: `ts/src/modules/FIR/`). Each register corresponds to one coefficient. Use `type: RW` so the host can write new values; set `reset` to the design-time default.
...
busAddressWidth: 32
registers:
  COEFF_0:  { address: "0x000", type: RW, reset: "0x00000000", description: "FIR tap 0 coefficient" }
  COEFF_1:  { address: "0x004", type: RW, reset: "0x00000000", description: "FIR tap 1 coefficient" }
  COEFF_2:  { address: "0x008", type: RW, reset: "0x00000000", description: "FIR tap 2 coefficient" }
  # ... one entry per tap
```
...
|---|---|---|
| `name` | yes | Used as the base name for generated TypeScript variables |
...
| `registers.<NAME>.address` | yes | Byte address as a hex string |
| `registers.<NAME>.type` | yes | `RW`, `RO`, `WO`, `RAM`, or `ROM` |
| `registers.<NAME>.reset` | no | Reset value as a hex string |
| `registers.<NAME>.description` | no | Human-readable description (appears in generated table) |
| `registers.<NAME>.fields.<F>.bits` | no | Bit range as `"MSB:LSB"` (e.g. `"3:1"`) |
| `registers.<NAME>.fields.<F>.reset` | no | Field reset value |
| `registers.<NAME>.fields.<F>.description` | no | Field description |
...
This produces **`ts/src/modules/FIR/stub.fir_coeffs.ts`** alongside the YAML (the `stub.*.ts` pattern is git-ignored). The file contains three things ready to paste into your module:

1. **`firCoeffsAddrMap`** — a `const` object mapping register names to `bigint` addresses.
2. **`firCoeffsDef`** — a `RegisterBlockDef` that describes types, widths, resets, and descriptions.
3. **`firCoeffs`** — a `RegisterBlock` instance constructed from the above.

It also prepends a **JSDoc markdown table** so the register map appears in TypeDoc output:
 ...
 * | -------- | ---------- | ---- | ----- | ---------- | ---------------------- |
 * | COEFF_0  | 0x00000000 | RW   | 32    | 0x00000000 | FIR tap 0 coefficient  |
 * | COEFF_1  | 0x00000004 | RW   | 32    | 0x00000000 | FIR tap 1 coefficient  |
 * ...
 */
export const firCoeffsAddrMap = { ... } as const
export const firCoeffsDef: RegisterBlockDef<...> = { ... }
export const firCoeffs = new RegisterBlock<...>(...)
```

The table only appears in TypeDoc when the `const` is **exported at module level**. If you paste the stub inside a class constructor (a local variable), the comment is still useful source-level documentation but TypeDoc won't index it.

---

### Step 3: Integrate into the module

Rather than using the generated stub as-is (which is for a fixed tap count), the FIR module builds its `RegisterBlockDef` **dynamically** from `this.params.coefficients` so any number of taps works. The generated stub acts as a reference for the register map layout. The integration pattern in `FIR.ts` is:

```typescript
// 1. Declare one coeff_N signal per tap (will receive register block outputs)
const coeffSigs: TSSV.Sig[] = []
for (let i = 0; i < numTaps; i++) {
  coeffSigs.push(this.addSignal(`coeff_${i}`, { width: 32, isSigned: true }))
}

// 2. Build the RegisterBlockDef dynamically
...
for (let i = 0; i < numTaps; i++) {
  ...
  coeffRegisters[`COEFF_${i}`] = {
    type: 'RW',
    reset: this.params.coefficients[i] ?? 0n,  // design-time default
    isSigned: true,
    width: 32,
...
// 3. Instantiate the RegisterBlock and wire it
const coeffRegBlock = new RegisterBlock<Record<string, bigint>>(
  { name: `${this.params.name}_coeffRegs`, busAddressWidth: 32 },
...
// autoBind=true, createMissing=true promotes the Memory bus to FIR ports
this.addSubmodule('coeff_block', coeffRegBlock, coeffBindings, true, true)

// 4. Use coeffSigs[i] in multipliers instead of the raw bigint
products.push(this.addMultiplier({ a: thisTap, b: coeffSigs[i] ?? new TSSV.Sig('0') }))
```

**Key points:**
- `addSubmodule(..., true, true)` — the second `true` (`createMissing`) auto-promotes the `regs` Memory bus interface up to the FIR module's ports. The FIR now has a `memory_32_32 regs()` interface that a host can drive.
- The `coefficients` parameter still sets the **reset values** of the registers. The filter behaves identically to a hard-coded coefficient FIR until a host writes new values.
- `coeffSum` (used for accumulator width sizing) still reads from `this.params.coefficients` — the output width is sized for the worst-case design-time values.

---

### What the generated SystemVerilog looks like

After running the FIR test (`node out/test/test_FIR.js`), `sv-examples/test_FIR_output/myFIR3.sv` contains:

- **`myFIR3_coeffRegs`** — a standalone module with 13 `COEFF_N` output ports, a `memory_32_32.inward regs` interface port, and one `always_ff` per coefficient register storing the host-written value (reset to the design-time default).
- **`myFIR3`** — the top FIR module, which instantiates `myFIR3_coeffRegs coeff_block(...)` and wires each `COEFF_N` output to an internal `coeff_N` signal used by the multipliers.
...
```bash
# 1. Write your YAML (in the module's directory)
#    ts/src/modules/FIR/fir_coeffs.yaml

# 2. Generate the TypeScript stub
./scripts/gen_regblock.sh ts/src/modules/FIR/fir_coeffs.yaml
#    → ts/src/modules/FIR/stub.fir_coeffs.ts  (reference; git-ignored)

# 3. Use the stub as a guide while writing the dynamic RegisterBlockDef
#    in your module's constructor (see FIR.ts for the full pattern)

# 4. Recompile and regenerate SV
